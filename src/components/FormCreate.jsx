...
export function FormCreate({handleChange, projectName, author, onclick, handleSubmit, description, status}) {
  return (
    <form className="p-5 border border-gray-300 rounded-md relative flex flex-col justify-between h-auto" onSubmit={handleSubmit}>
      <div className="flex flex-col gap-3">
        <div className="grid grid-cols-3">
          ...
          <div className="flex flex-col items-end gap-2 justify-start col-span-1">
            <button type="button" className="hover:border rounded-lg p-1 hover:border-orange-300" onClick={onclick}>
              <Trash className="w-5" />
            </button>
          </div>
        </div>
      ...
      <div className="flex items-center justify-between mt-7">
        ...
        <div className="px-2 text-sm bg-pink-300 rounded-l-full absolute right-0">
          <select 
            className="bg-pink-300 text-xs opacity-70 p-0.5 rounded-l-full focus:outline-none"
            onChange={handleChange}
            value={status}
            name="status"
          >
